<template>
  <div
    class="w-full h-5/6 overflow-y-scroll none-scroll pt-3 relative"
    id="invoice"
  >
    <div class="bg-gray-200 h-max text-sm font-thin p-5 grid grid-cols-2">
      <div class="mb-3 flex flex-col justify-between border-r-2 mr-3">
        <div class="mb-2">
          <p class="font-bold text-primary mr-5">Create invoice for:</p>
          <p class="text-primary font-sm">contract #{{ contract }}</p>
        </div>
        <div class="mb-2">
          <p class="font-bold text-primary mr-5">
            Pricing:
            <span
              class="bg-green-500 rounded-full px-2 py-1 text-xs text-white"
              >{{ pricing }}</span
            >
          </p>
          <p class="text-primary font-sm">₱{{ amount }}</p>
        </div>

        <div class="mb-2">
          <p class="font-bold text-primary mr-5">Account Executive:</p>
          <p class="text-primary font-sm">{{ ae }}</p>
        </div>
      </div>
      <div class="mb-3 flex flex-col justify-between">
        <div class="pb-5 border-b-2">
          ...
          <input
            ...
            :class="
              invFrom && invTo && invFrom > invTo
                ? 'outline outline-red-500 bg-red-300'
                : 'bg-secondary'
            "
            @change="setTotal"
            v-model="invFrom"
        ...
        <div>
          ...
          <input
            type="text"
            class="focus-visible:outline-2 w-full font-thin text-sm px-2 py-1 shadow-md rounded-r-md text-primary focus-within:bg-secondary mb-2"
            :class="
              invoiceExists
                ? 'outline outline-red-500 bg-red-300 focus-within:outline-red-500'
                : 'bg-secondary focus-within:outline-primary'
            "
            v-model="invNum"
          ...
          <input
            type="date"
            class="bg-secondary focus-visible:outline-2 w-full focus-within:outline-primary font-thin text-sm px-2 py-1 shadow-md rounded-r-md text-primary focus-within:bg-secondary"
    ...
    <div class="mb-3">
      <div class="w-full h-min px-4 py-5 bg-yellow-100 m-2" v-if="isValid">
        <p class="text-sm text-yellow-500 text-justify">
          You are about to bill:
          ...
          <span v-if="pricing === 'monthly'"
            >({{ months }} months duration)</span
          >
          <span v-else-if="pricing === 'daily'"
            >({{ days }} days duration)</span
          >
          <span v-else-if="pricing === 'fixed'"
            >({{ days }} days duration)</span
          >
        </p>
      </div>
    ...
    <div class="w-full flex justify-center my-2">
      <button
        class="font-bold px-2 py-1 rounded-lg text-white hover:bg-active"
    ...
    <table class="w-full mt-2">
      <thead class="bg-primary text-white text-sm font-bold">
        <tr>
          <td>
            <i class="block h-4 w-4"><PrinterIcon /></i>
          </td>
          <td>Invoice</td>
          <td>Applicable</td>
          <td>Amount</td>
      ...
      <tbody class="text-primary text-sm font-normal">
        ...
        >
          <td>
            <a :href="baseURL + invoice.file" class="hover:text-green-500">
          ...
          <td class="">
            <p class="text-xs">
              F:
              <span class="font-bold">{{ invoice.applicable_month_from }}</span>
            </p>
            <p class="text-xs">
              T:
              <span class="font-bold">{{ invoice.applicable_month_to }}</span>
            </p>
          </td>
          <td>₱ {{ amount }}</td>
          <td>
            <div v-if="invoice.paid">
              ...
              <p class="text-xs font-thin">
                {{ invoice.paid ? "OR#" + invoice.or_number : "" }}
              </p>
            </div>
            <div v-else class="relative">
              <AdPayment
                :invoice="invoice"
                @submitted="getInvoiceList(contract)"
              />
            </div>
          </td>
          <td>
            <InvoiceDepositSlip v-if="invoice.deposited" :invoice="invoice" />
            <InvoiceDeposit
              v-else
...
const checkValidity = setInterval(() => {
  const allFilled =
    invNum.value && invDate.value && invFrom.value && invTo.value;
