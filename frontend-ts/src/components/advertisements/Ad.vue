<template>
  ...
  <div
    ...
    class="bg-white border shadow-md h-full w-full rounded-xl hover:shadow-xl overflow-hidden flex flex-col items-center py-5 px-5 relative"
    :class="
    ...
    "
  >
    <span class="flex h-3 w-3 absolute top-1 right-1" v-if="codeOrange">
      <span
    ...
    <span class="flex h-3 w-3 absolute top-1 right-1" v-else-if="codeRed">
      <span
    ...
    <div class="grid grid-cols-2 mt-3 w-full h-full relative">
      <div
        class="absolute w-full h-full bg-white opacity-80 top-0 bottom-0 left-0 right-0"
        v-if="daysLeft < 0"
      ></div>

      <div class="h-24 w-24 overflow-hidden rounded-md">
        <img
          :src="ad.ad_avatar ? baseURL + ad.ad_avatar : StaticAdPhoto"
      ...
      <div class="flex flex-col justify-center">
        ...
        <p class="text-xs text-center">
          spot{{ adVersionView.ad_spots > 1 ? "s" : "" }} per day
        </p>
      </div>
    </div>
    <div class="border-t-2 w-full flex justify-end">
      <button
        class="text-xs font-bold text-right mx-5"
  ...
  <teleport to="body">
    ...
    >
      <div
        class="bg-white rounded-md w-adcontract h-5/6 px-5 py-5 relative"
        id="mainContract"
      >
        <!-- Options -->
        <div class="w-32 absolute top-2 right-2 flex flex-col">
          ...
          <div
            class="bg-white w-full h-max flex flex-col items-start shadow-md border"
            v-if="adMenu"
          ...
          >
            ...
            <button
              class="w-full border-b text-start hover:bg-gray-500 hover:text-white px-2 text-sm"
              @click="
                contractView = 'edit';
                adMenu = false;
              "
        ...
        <div
          class="absolute top-2 left-2 flex flex-col"
          v-if="contractView !== 'main'"
        >
        ...
        <!-- MAIN VIEW -->
        <div
          class="w-full h-5/6 overflow-y-scroll none-scroll pb-3"
          v-if="contractView === 'main'"
        >
          <div class="pb-2">
          ...
          <div class="grid grid-cols-2 gap-2">
            <div
              class="h-full w-auto overflow-hidden rounded-md relative flex shadow"
            >
              <img
                v-if="adAvatarUrl"
                :src="adAvatarUrl"
              ...
              <button
                class="absolute font-sans text-lg font-black text-white bg-black bg-opacity-30 w-full h-full"
              ...
              <input
                ...
                ref="adAvatarFile"
                accept="image/*"
                @change="uploadAndView"
                capture
              />
              <button
                v-if="adAvatarUrl"
                class="absolute top-1 right-1 bg-primary text-white font-bold text-sm px-2 py-1 rounded-md"
              ...
              <button
                v-else
                class="absolute top-0 right-0 text-primary hover:text-primary font-bold text-sm px-2 py-1 rounded-md"
            ...
            <div class="flex flex-col justify-around border-l px-1">
              <p class="text-sm font-thin my-1">
                Type: <span class="font-bold">{{ ad.type }}</span>
              </p>
              <p class="text-sm font-thin my-1">
                BO: <span class="font-bold">{{ ad.bo_number }}</span>
              </p>
              <p class="text-sm font-thin my-1">
                Starts:
                <span class="font-bold">{{
                  adVersionView.broadcast_start
                }}</span>
              </p>
              <p class="text-sm font-thin my-1">
                Ends:
                <span class="font-bold">{{ adVersionView.broadcast_end }}</span>
              </p>
              <p class="text-sm font-thin my-1">
                Pricing:
                <span class="font-bold">{{ adVersionView.pricing }}</span>
              </p>
              <p
                class="text-sm font-thin my-1"
                v-if="adVersionView.pricing === 'fixed'"
              >
                Amount:
                <span class="font-bold">₱{{ adVersionView.amount }}</span>
              </p>
              <p class="text-sm font-thin my-1" v-else>
                Amount:
                <span class="font-bold"
                  >₱{{ adVersionView.amount }}/{{
                    adVersionView.pricing === "monthly" ? "month" : "day"
                  }}</span
                >
              </p>
              <p class="text-sm font-thin my-1">
                Airing:
                <span class="font-bold"
                  >{{ adVersionView.ad_spots }} spots/day</span
                >
              </p>
              <p class="text-sm font-thin my-1">
                Account Executive: <span class="font-bold">{{ ae }}</span>
              </p>
            </div>
          </div>

          <div class="my-3 px-5 py-2 bg-red-100" v-if="codeRed">
            <p class="text-sm text-red-500">
              Will end in {{ 24 + hoursLeft }} hour{{
                24 + hoursLeft > 1 ? "s" : ""
              }}
            </p>
          </div>
          <div class="my-3 px-5 py-2 bg-orange-100" v-else-if="codeOrange">
            <p class="text-sm text-orange-500">
              Will end in {{ daysLeft }} day{{ daysLeft > 1 ? "s" : "" }}!
            </p>
          </div>
          <div class="my-3 px-5 py-2 bg-gray-100" v-else-if="codeBlack">
            <p class="text-sm text-gray-500">
              Inactive since {{ expiredWhen }}!
            </p>
          </div>
          <div
            class="my-3 px-5 py-2 bg-green-100 flex justify-between items-center"
            v-else
          >
            <p class="text-sm text-green-500">Up and running</p>
            <button>
              <i class="flex w-5 h-5 items-center text-green-500"
                ><StackIcon
              /></i>
            </button>
          </div>
          ...
          <div class="w-full h-max" v-if="adVersionView.has_tagline">
            ...
            <div class="sched-grid h-max p-5 bg-gray-200 my-3">
              <div
                v-if="adVersionView.aob_sched"
                v-for="(aob, index) in aobScheduleList"
                :key="index"
              ...
              >
                <div
                  class="tagline h-max w-max px-1 bg-primary text-white-500 flex items-center text-center justify-center"
                >
                  AOB
                </div>
                {{ aob }}
              ...
              <div
                v-if="adVersionView.tc_sched"
                v-for="(tc, index) in tcScheduleList"
                :key="index"
                class="relative w-max text-sm text-center bg-primary hover:bg-active text-white px-2 py-1 rounded-full m-2 place-self-center"
              >
                <div
                  class="tagline h-max w-max px-1 bg-green-500 text-white-500 flex items-center text-center justify-center"
                >
                  TC
                </div>
                {{ tc }}
              ...
              <div
                v-if="adVersionView.ss_sched"
                v-for="(ss, index) in ssScheduleList"
                :key="index"
                class="relative w-max text-sm text-center bg-primary hover:bg-active text-white px-2 py-1 rounded-full m-2 place-self-center"
              >
                <div
                  class="tagline h-max w-max px-1 bg-orange-500 text-white-500 flex items-center text-center justify-center"
                >
                  SS
                </div>
                {{ ss }}
          ...
          <div class="w-full">
            ...
            <div class="h-max p-5 bg-gray-200 my-3">
              <AudioItem
                v-for="audioFile in adVersionView.files"
                :id="audioFile"
                :key="audioFile"
                class="w-11/12 mx-auto my-3"
              />
            </div>
          </div>
        </div>
...
const getSchedules = () => {
  ...
  aobScheduleList.value =
    adVersionView.value.has_tagline && adVersionView.value.aob_sched
      ? adVersionView.value.aob_sched.split(",")
      : [];
  tcScheduleList.value =
    adVersionView.value.has_tagline && adVersionView.value.tc_sched
      ? adVersionView.value.tc_sched.split(",")
      : [];
  ssScheduleList.value =
    adVersionView.value.has_tagline && adVersionView.value.ss_sched
      ? adVersionView.value.ss_sched.split(",")
      : [];
};
const ae = ref("");
const getAE = async () => {
