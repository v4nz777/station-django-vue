<template>
  <div
    class="md:w-96 w-screen h-max bg-white p-3 shadow-lg rounded-2xl my-5 flex justify-between px-5"
  >
    <div>
      <p class="text-lg font-bold mb-2 text-gray-600">
        {{ dtInterrupted.complete_date }}
      </p>
      <p class="text-sm">
        Interrupted:
        <span class="text-sky-500 font-bold">{{
          dtInterrupted.complete_time
        }}</span>
      </p>
      <p class="text-sm">
        Restored:
        <span class="text-sky-500 font-bold">{{
          dtRestored.complete_time
        }}</span>
        <span
          class="text-gray-500 text-xs italic"
          v-if="dtRestored.complete_date !== dtInterrupted.complete_date"
        >
          ({{ dtRestored.complete_date }})</span
        >
      </p>
    </div>
    <div class="w-max flex flex-col items-center justify-center">
      <p class="text-red-500 font-bold" v-if="datum.duration_hours">
        ...
        {{ datum.duration_minutes }} min{{
          datum.duration_minutes > 1 ? "s" : ""
        }}
      </p>
      <p class="text-red-500 font-bold" v-else>
        {{ datum.duration_minutes }} minute{{
          datum.duration_minutes > 1 ? "s" : ""
        }}
      </p>

      <div
        v-if="datum.scheduled"
        class="text-xs font-bold bg-green-500 text-white w-max text-center px-2 py-1 rounded-full"
      >
        SCHEDULED
      </div>
      <div
        v-else
        class="text-xs font-bold bg-yellow-500 text-white w-max text-center px-2 py-1 rounded-full"
      >
        UNSCHEDULED
      </div>
    </div>
