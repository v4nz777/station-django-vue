<template>
  <div class="h-full w-max flex flex-col px-16 py-14">
    <div
      class="bg-white text-lg text-center text-gray-500 font-bold px-5 py-2 border-b border-gray-300"
    >
      Daily Time Records
    </div>
    <div
      class="flex justify-around items-start bg-white text-gray-500 font-normal px-5 py-0 border-b border-gray-300 text-sm"
    >
      <button
        class="px-2 w-full mx-1 rounded-b-lg flex flex-col text-center"
        @click="selected = 'records'"
      >
        <span
          class="w-full mx-auto my-3 px-3 py-2"
          :class="
            selected === 'records'
              ? 'text-red-500'
        ...
        >
        <div
          class="w-full h-1 rounded-t-md"
          :class="selected === 'records' ? 'bg-red-500' : 'bg-white'"
        ></div>
      </button>
      <button
        class="px-2 w-full mx-1 rounded-b-lg flex flex-col text-center"
      ...
      >
        ...
        <div
          class="w-full h-1 rounded-t-md"
          :class="selected === 'export' ? 'bg-red-500' : 'bg-white'"
        ></div>
      </button>
    </div>

    <div
      class="w-dtr h-screen shadow bg-white overflow-auto red-scroll"
    ...
    >
      <div class="bg-red-100 border flex justify-center">
        <input
          ...
          id="dtrFilterMonth"
          type="month"
          class="my-1 px-2 rounded-md text-red-500 font-thin shadow-lg"
        />
      </div>
      <table class="w-full">
        <tbody class="">
          <tr
            class="relative text-justify transform scale-100 text-sm py-1 border-b-2 border-red-100 cursor-default"
          >
          ...
          <tr
            class="relative text-justify transform scale-100 text-xs py-1 border-b-2 border-red-100 cursor-default"
    ...
    <!--:::::::::::::::: TOGGLE SEPARATOR:::::::::::::::::::::::::::::::::::::::-->
    <div
      class="w-dtr h-screen shadow bg-red-100 overflow-auto flex justify-center items-center"
      v-else
    >
      <div
        class="h-96 w-72 bg-white border shadow-md rounded-lg flex flex-col items-center"
      >
        ...
        <div class="my-5 text-right">
          ...
          <input
            type="date"
            id="from"
            class="bg-red-300 font-thin text-sm px-2 py-1 shadow-md rounded-sm"
            v-model="from"
          />
          <div class="my-5"></div>
        ...
        <div class="my-5 flex flex-col justify-center items-center">
          ...
          <Loader v-if="loading" />
          <button
            class="hover:bg-red-500 bg-red-400 text-white text-2xl px-5 py-2 mt-5 rounded-full w-min"
            @click="generate"
            v-else
          >
            Generate
          ...
          <div
            class="flex flex-col mt-8 py-1 px-2 hover:bg-white bg-green-500 rounded-sm"
            v-if="file"
            @mouseover="dl_btn_hovered = true"
            @mouseout="dl_btn_hovered = false"
          >
            ...
            <a
              :href="file"
              class="font-light text-green-500 text-xs"
              v-if="dl_btn_hovered"
              >{{ fileName }}</a
            >
          </div>
