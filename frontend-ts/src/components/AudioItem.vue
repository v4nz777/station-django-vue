<template>
  <div class="w-full flex my-1" v-if="isPresent">
    <button
      class="items-center align-bottom hover:text-blue-500 relative z-20"
    ...
    >
      ...
      <div
        v-if="clicked"
        class="rounded-full w-6 h-6 bg-sky-500 animate-ping absolute top-0 bottom-0 left-0 right-0"
      ></div>
    </button>
    <div
      class="flex items-center justify-between w-full bg-gray-300 rounded-r-full relative shadow-xl"
    >
      <span class="align-center text-sm font-light ml-2"
        >{{
          name.length > 25
            ? name.substring(0, 25) +
              "..." +
      ...
      <span v-if="duration" class="text-xs font-semibold mr-2">{{
        duration - current + "s"
      }}</span>
      <div
        class="absolute top-0 bg-red-300 h-full rounded-r-full"
        :style="
          played
            ? { animationName: 'loadingfx', animationDuration: duration + 's' }
            : {}
        "
        id="duration"
      ></div>
