<template>
  <HeaderTab
    class="flex justify-between px-3 bg-primary flex-wrap text-white font-medium b-border shadow-lg"
    v-if="userstore.user"
  />

  <div class="flex flex-row h-screen w-screen fixed bg-secondary">
    <SideBar class="flex-shrink md:flex hidden" v-if="userstore.user" />

    <div
      class="overflow-y-scroll mx-auto w-full flex justify-center mb-14"
      id="mainview-scroll"
    >
      <RouterView />
    </div>
