<template>
  <div class="md:w-full h-screen md:px-24">
    ...
    >
      <div class="flex flex-col md:flex-row gap-2 w-max">
        <div
          ...
          <img :src="user.avatar" class="object-cover w-fill h-full" />
        </div>
        <div class="">
          <p
          ...
          >
            {{
              user.userDetails.first_name.charAt(0).toUpperCase() +
              user.userDetails.first_name.slice(1)
            }}
            {{
              user.userDetails.last_name.charAt(0).toUpperCase() +
              user.userDetails.last_name.slice(1)
            }}
          </p>
          ...
          >
            <p class="text-xs font-bold text-white">
              {{ user.userDetails.position ?? "Unassigned" }}
            </p>
          </div>
        </div>
      </div>
    ...
    <div class="w-full mt-4 mb-14 flex flex-col px-4 md:mx-10">
      ...
      <div class="flex my-6" v-for="i in activitystore.filteredActivities" :key="i.id">
        <!--:: Main Activities ::-->
        <div
