<template>
  <div class="md:w-full md:mx-20 w-screen flex flex-col items-center">
    <div
      class="md:w-full w-screen h-max bg-white rounded-3xl my-5 py-2 shadow-xl shadow-yellow-500 flex"
    >
      <NewTransmitterLog @new="filterLogs(today)" />
    </div>

    <div class="w-full md:flex md:justify-center h-max px-5 pt-5 pb-12">
      <TransmitterLogTable :logs="logs" :filter="today" @search="filterLogs" />
    </div>
  </div>
</template>
