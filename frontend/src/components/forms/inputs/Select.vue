<template>
    <div class="my-3 w-96 text-right px-11">
        ...
        <div class="inline-block">
            <select v-model="value" class="bg-secondary focus-visible:outline-2 
                focus-within:outline-primary font-thin text-sm px-2 py-1 shadow-md rounded-r-md
                text-primary focus-within:bg-secondary"
                @change="setVal">
            <option v-for="offl in offlineOptions" :value="offl">{{offl}}</option>
            <option v-for="onl in onlineOptions" :value="onl[onlineKey]">{{onl[onlineKey]}}</option>
            </select>
