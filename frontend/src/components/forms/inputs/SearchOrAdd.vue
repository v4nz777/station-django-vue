<template>
    <div class="my-3 w-full text-right px-11"
        ...
        <div class="inline-block relative">
            <label class="text-white px-2 py-1 w-full rounded-l-md
                shadow-md text-sm mr-1"
                :class="value? 'bg-primary text-white':'bg-secondary text-primary' ">{{title}}: </label>
            <input type="text" v-model="value" class="bg-secondary focus-visible:outline-2 
            ...
            <ul class="bg-white absolute w-full h-max shadow-lg" v-if="searchList.length">
                <li v-for="item in searchList" class="text-sm text-left font-light
                hover:text-white hover:bg-primary w-full p-1 z-50 cursor-pointer"
...
<script setup>
    ...
    const emptyList = ()=> searchList.value = [];chosen.value=false;
    const getList = async () => {
    ...
    watch(()=>props.default,(newVal,old)=> {
        select(newVal)
