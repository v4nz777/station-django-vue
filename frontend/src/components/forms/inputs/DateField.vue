<template>
    <div class="my-3 w-full text-right px-11"
        :class="end?'border-b mb-2 pb-3':''">
        <label for="adTitle" class="text-white px-2 py-1 w-full rounded-l-md
            shadow-md text-sm mr-1"
            :class="value? 'bg-primary text-white':'bg-secondary text-primary'">{{title}}: </label>
        <input type="date" v-model="value" class="bg-secondary focus-visible:outline-2 
        ...
        <div class="inline-block ml-1" v-if="value">
            <i class="inline-block w-4 h-4 text-primary" v-if="valid"><CheckCircleIcon/></i>
...
const props = defineProps({
    ...
    placeholder:String,
    match:String // Useful for comparing dates
...
const compareWith = (dateString)=> {
    // will return the difference between two dates in days
    if(dateString){
        const xSubject = moment(value.value)
        const xWith = moment(dateString)
        return xSubject.diff(xWith,'days')
    }
}
...
const decide = async()=> {
    if(!props.match){
        ...
        const difference = compareWith(props.match)
        await verify()
        if(difference >=0) valid.value = true
        else valid.value = false
        emits("done", value.value,valid.value)
    }
}
...
watch(()=>props.default,async (newDefault,old)=> {
    value.value=newDefault
    await decide()
})
watch(()=>props.match,async ()=> {
    await decide()
})
</script>