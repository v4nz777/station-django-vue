<template>
    <div class="my-3 w-96 text-right px-11">
        ...
        <label class="text-white px-2 py-1 w-full rounded-l-md shadow-md text-sm mr-1"
            :class="uploaded.length? 'bg-primary text-white':'bg-secondary text-primary' ">{{title}}</label>
        <input type="file" ref="fileFieldInput" :accept=accepts class="hidden" multiple @change="fileUpload">
        <button class="bg-secondary focus-visible:outline-2 
            ...
            text-primary focus-within:bg-secondary border border-primary border-dashed"
            @click="$refs.fileFieldInput.click">Upload</button>
        ...
        <ul class="text-xs text-primary text-right mx-1"
            ...
            <li v-for="f,index in uploaded">
                {{f.name.length > 25? `${f.name.substring(0,25)}...${f.name.substring(f.name.length-3,f.name.length)}`: f.name}}
                <button @click="removeFromFiles(f,index)"
                    class="text-primary hover:bg-red-500 hover:text-white rounded-full w-4 h-4">
                    <p class="font-bold flex justify-center items-center w-full h-full">X</p>
                </button>
            </li>
        </ul>
