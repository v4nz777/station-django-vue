<template>
    <div class="m-2 w-36 h-36 relative">
        ...
        <button class="z-10 absolute bottom-custom w-28 h-12 left-2"
            @click="open = true">
            <div class="shadow-md w-full h-full bg-sky-500 bg-opacity-80 hover:bg-opacity-100 rounded-lg"
                v-if="equipment.owner === 'mbc'">
                <span  class="text-xs text-white px-1 rounded font-semibold">MBC</span>
                <p class="text-sm text-center font-bold rounded-md text-white">{{equipment.name}}</p>
            </div>
            <div class="shadow-md w-full h-full bg-emerald-500 bg-opacity-80 hover:bg-opacity-100 rounded-lg"
                v-else-if="equipment.owner === 'partner'">
                <span  class="text-xs text-white px-1 rounded font-semibold">Partner</span>
                <p class="text-sm text-center font-bold rounded-md text-white">{{equipment.name}}</p>
            </div>
                ...
                <div class="bg-white rounded-md w-adcontract h-5/6 px-5 py-5 relative">
                    <div class="w-full h-max flex justify-around border-b-2 pb-2">
                        ...
                        <div class="w-max h-max">    
                            <div class="bg-emerald-100 w-32 h-60 overflow-y-scroll none-scroll">
                            ...
                            <div class="bg-emerald-100 w-full h-9 relative">
                                ...
                                <div class="absolute bg-white w-72 h-52 right-0 shadow-md grid shadow-emerald-100"
                                    ...
                                    <div class="p-3 small-grid-auto-cols overflow-scroll none-scroll">
                                        ...
                                        <div v-for="imgurl in uploadGalleryListForShow" class="overflow-hidden shadow-md w-9 h-12">
                                            <img :src="imgurl" :alt="imgurl.file_name" srcset="" class="w-full h-full object-cover">
                                        </div>
                                    </div>
                                    <div class="w-full flex">
                                        <button class="my-2 mx-auto flex justify-center items-center text-white w-28 px-5 h-9 font-bold shadow-md bg-red-500 hover:bg-red-600"
                                            @click="uploadGallery=null;uploadGalleryList=[];uploadGalleryListForShow=[];">
                                            Cancel
                                        </button>
...
<script setup>
    ...
    const eqGalleryUpload = async ()=> {
        ...
        uploadGalleryList.value.forEach((upload)=> fd.append("uploads",upload,upload.name))
    ...
    const setAsDisplay = async ()=> {
        ...
        const response = await axios.put(`set_eq_avatar/${props.id}/${eqPicPreview.value.id}`)
    ...
    const getEquipment = async ()=> {
        ...
        if(!avatar.value.file&&eqGallery.value.length&&!eqPicPreview.value.file){
...
*/  watch(()=>equipment.value.gallery?equipment.value.gallery.length:0, async (newval,oldval)=>{
...
*/  watch(()=>equipment.value.avatar,async(newval,oldval)=> {
        if(newval !== oldval){
...
*/  watch(()=>props.mother.equipments.length,async(newval,oldval)=>{
        if(newval !== oldval){      
            await getEquipment()
            setTimeout(async () => {
                await getBrand()
 ...
 */
    watch(()=>props.alled,async(newval,oldval)=>{
        if(props.alled){
            marked.value=true
        }else marked.value=false
    })
...
*/
    watch(()=>equipment.value.status,async(newval,oldval)=>{
        if(props.batchedItems.includes(props.id)){
            marked.value=true
        }else marked.value=false
    })
