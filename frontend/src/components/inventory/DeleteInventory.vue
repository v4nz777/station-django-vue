<template>
    <div class="relative">
        ...
        <button @click="open = true" @mouseover="parentIsHovered = true" @mouseout=" parentIsHovered = false"
                    ...
                    :class="items.length?'hover:shadow-md hover:bg-red-500 hover:text-white':''"
                    class="w-max rounded-md  flex items-center justify-start px-2 h-9">

            <i v-if="!items.length" class="h-6 w-6 text-gray-500"><TrashIcon/></i>
            <i v-else class="h-6 w-6" :class="parentIsHovered ? 'text-white':'text-red-500'"><TrashIcon/></i>
        ...
        <div class="bg-slate-800 bg-opacity-50 flex justify-center items-center absolute top-0 right-0 bottom-0 left-0"
            ...
            <div id="inventory-scroll" class="w-inventory-add h-96 bg-white overflow-y-scroll pb-8 flex flex-col items-center justify-between">
                <p class="m-3 font-bold">
                    Are you sure you want to delete {{itemsReactive.length>1?'these items':'this item'}}?
                </p>
                <ul class="text-start">
                    <li v-for="item in itemsReactive">🟦{{item.name}}</li>
                </ul>
                <!-- <div class="flex mt-5 justify-between w-full px-28">
                    <p>Move to:</p>
                    <select v-model="group" class="border">
                        <option v-for="g in groups" :value="g.id"><p class="font-bold">{{g.group_name?g.group_name:'ungrouped'}}</p></option>
                    </select>
                </div>
                <p class="mb-5 text-xs text-gray-800">select group</p> -->

                
                <button @click="deleteEquipments()" class="bg-red-500 font-bold text-white px-4">
                    CONFIRM DELETE
                </button>
            </div>
            <!-- end of inside modal -->
...
<script setup>
    ...
    import { ArchiveIcon, ArrowCircleRightIcon, TrashIcon } from "@heroicons/vue/solid";
    ...
    const deleteEquipments = async ()=> {
        ...
        props.items.forEach((eq)=> {
            fd.append("delete_equipments",eq)
        })
        const response = await axios.post("delete_items",fd)
