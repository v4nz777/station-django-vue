<template>
    <!-- <a href="#" @click="go" class="text-gray-500 rounded-md m-1 hover:bg-gray-500 hover:text-white"> -->
    <div class="flex justify-between rounded-md m-1 px-1"
        :class="parentIsHovered || selected?'bg-gray-500':''"
        @mouseover="parentIsHovered = true" @mouseout="parentIsHovered = false">
        <i class="h-6 w-6 py-2" :class="parentIsHovered || selected?'text-white':'text-gray-500'">
            <slot name="icon"></slot>
        </i>
        <div class="justify-self-start text-middle py-2 font-bold w-28 flex justify-between">
            <p class="text-start" :class="parentIsHovered || selected?'text-white':'text-gray-500'">{{title}}</p>
            <div class="relative">
                <button @click="toggleOpen">
                    <i class="block h-6 w-6" :class="parentIsHovered || selected?'text-white':'text-gray-500'">
                        <ChevronRightIcon/>
                    </i>
                </button>
                <ul v-if="menuOpen && links.length" class="absolute top-1 left-8 bg-white shadow-md w-32">
                    <li v-for="link in links" class="hover:bg-gray-500 hover:text-white text-sm font-normal px-2 border-b"
