<template>
    <div class="md:w-96 w-full h-max bg-white p-3 shadow-lg rounded-2xl my-5 mx-auto flex justify-between">
        <div>
            ...
            <p class="text-sm">Time: <span class="text-sky-500">{{dt.complete_time}}</span></p>
        </div>
        <div class="w-max flex items-center justify-center">
        
            <i v-if="datum.trend === 'up'" class="block w-9 h-9 text-red-500"><TrendingUpIcon/></i>
            <i v-else-if="datum.trend === 'down'" class="block w-9 h-9 text-green-500"><TrendingDownIcon/></i>
            <i v-else-if="datum.trend === 'same'" class="block w-9 h-9 text-sky-500"><ArrowNarrowRightIcon/></i>


            <p v-if="datum.trend === 'up'" class="text-lg text-red-500 font-bold">{{datum.consumed}} kwH</p>
            <p v-else-if="datum.trend === 'down'" class="text-lg text-green-500 font-bold">{{datum.consumed}} kwH</p>
            <p v-else-if="datum.trend === 'same'" class="text-lg text-sky-500 font-bold">{{datum.consumed}} kwH</p>
