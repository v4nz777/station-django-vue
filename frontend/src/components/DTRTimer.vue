<template>
    <button @click="logoutToDTR" v-if="logged"
        class="bg-red-500 text-white font-bold text-sm px-2 py-1 rounded-full">
        Time Out
    </button>
    <div class="flex bg-white rounded-2xl b-border shadow-md justify-center items-center px-2 h-9">
        <p class="justify-self-start py-2" v-if="logged">
            ⏱️<span class="text-red-500 text-sm font-normal">{{dtrstore.formatLogTimer}}</span>
        </p>
        <p class="justify-self-start py-2 cursor-pointer" v-else
            @click="loginToDTR">
